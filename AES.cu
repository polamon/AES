...
__global__ void d_multiply(unsigned short int *d_a, unsigned short int *d_b) {

	int index = 320 * 32 * blockIdx.y + 320 * threadIdx.y + 32 * blockIdx.x + threadIdx.x;
	if (index >= 100000) return;

	unsigned short int d_c = 0;
	for (int i = 7; i >= 0; i--) {
		d_c <<= 1;
		if (d_c & 256) d_c ^= PX;
		if (d_b[index] & (1 << i)) d_c ^= d_a[index];
	}
	d_a[index] = d_c;
}

__global__ void d_inversion(unsigned short int *d_a) {

	int gindex = 320 * 32 * blockIdx.y + 320 * threadIdx.y + 32 * blockIdx.x + threadIdx.x;
	unsigned short int lindex = threadIdx.y * 32 + threadIdx.x;

	__shared__ unsigned short int matrix[256];

	if (threadIdx.y < 8) matrix[lindex] = d_a[100000 + lindex];

	__syncthreads();

	if (gindex >= 100000) return;
	d_a[gindex] = matrix[d_a[gindex]];
}
...
// sBox Linear Analysis in normal way
	...
	cout << "uSeconds: "
		...
		+ end.tv_usec - begin.tv_usec << endl << endl;
...
//	cout << "uSeconds: " << (end.tv_sec - begin.tv_sec) * 1000000 + end.tv_usec - begin.tv_usec << endl << endl;
...
void GPUMultiply()
{
	...
	cout << "Multiply for 100,000 times with GPU." << endl;

	int memSize = 100000 * sizeof(unsigned short int);
	
	unsigned short int *a, *b, *c;

	a = (unsigned short int *)malloc(memSize);
	b = (unsigned short int *)malloc(memSize);
	c = (unsigned short int *)malloc(memSize);
	...
	d_multiply<<<grid, threads>>>(d_a, d_b);
...
//check
	...
	cout << "Time(uSeconds): " << (end.tv_sec - begin.tv_sec) * 1000000 
		+ end.tv_usec - begin.tv_usec << endl << endl;
...
void GPUInversion()
{
	...
	cout << "Inversion for 100,000 times with GPU." << endl;

	int memSize = (100000 + 256) * sizeof(unsigned short int);
	...
	for (int i = 0; i < 256; i++) {
		a[i + 100000] = inversionMatrix[i];
	}

	gettimeofday(&begin, 0);

	unsigned short int *d_a;
	...
	d_inversion<<<grid, threads>>>(d_a);

	cudaDeviceSynchronize();

	cudaMemcpy(b, d_a, memSize, cudaMemcpyDeviceToHost);
...
//check
	for (int i = 0; i < 100000; i++)
		if ((a[i] == 0 && b[i] != 0) || (a[i] != 0 && multiply(a[i], b[i]) != 1)) {
			cout << "Error " << i << endl;
			break;
		}
...
int main()
{
	...
	unsigned short int *temp;
	cudaMalloc((void**)&temp, 100 * sizeof(unsigned short int));
	cudaFree(temp);
